...
| D7 | [Pirt maintenance/yield](D7_pirt/D7_pirt.pdf) | 2026-08-08 | **No** — maintenance and yield cannot be separated from the data in hand, and a Pirt fit here is a **reparameterisation of Fig 6, not a new result**. Both axes carry *r* by construction (G = 60qr exactly; R = K·O₂ref/N₀ with N₀ ∝ e^(−rΔt)), and holding N₀ fixed makes the Pirt slope change *sign* — 9.13→5.52→−0.59 across the 15 taxa, −2.10→+7.50 in *Pseudomonas*. Fig 6 already implied this: the elasticity d log R/d log G cannot exceed 1 for non-negative maintenance, and the published 0.283 inverts to 3.79, so the direct fit duly returns m = −553. Even setting the artefact aside the split is weakly identified (intercept–slope corr −0.93; 32% of simulations return negative m). A wider growth range is necessary but **not sufficient** — the binding constraint is residual scale, needing n ≈ 9,900 against 75. |
| D8 | [Balance and mass balance](D8_balance/D8_balance.pdf) | 2026-08-08 | The growth–respiration **balance is secure without resolving the absolute scale**: E_r = 0.60 eV, E_K = 0.69 eV, E_K − E_r = 0.09 eV [0.02, 0.29] (respiration more temperature-sensitive, interval excludes 0), balance optimum 25.3 °C [24.0, 26.1], 5.2-fold shift across 20–40 °C, and taxon rankings — none touching FC_TO_CELLS_PER_L, the quota, RQ or absolute N₀. The published T_opt(CUE) of 32.44 °C sits **7.09 °C above** that, and not because of scale: +2.5 °C from the 45-min N₀ back-projection, +4.6 °C from the Sharpe–Schoolfield functional form. **An independent mass-balance CUE agrees to within 8%** (ratio 1.077, IQR 0.044; 0/75 physically impossible), needing no N₀ at all — a genuine external check the kinetic method passes. |
